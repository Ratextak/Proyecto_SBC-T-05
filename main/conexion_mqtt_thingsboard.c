...
esp_mqtt_client_handle_t cliente;  // Cliente MQTT.
...
static void mqtt_app_start(void) {
    esp_mqtt_client_config_t mqtt_cfg = {
        ...
        .port = 1883,
        .username = THINGSBOARD_TOKEN,  // Token del dispositivo.
    };

    cliente = esp_mqtt_client_init(&mqtt_cfg);
    esp_mqtt_client_register_event(cliente, ESP_EVENT_ANY_ID, mqtt_event_handler, cliente);
...
void mqtt_mandar_credenciales_telegram(void) {
    ...
    esp_mqtt_client_publish(cliente, "v1/devices/me/attributes", post_data, 0, 1, 0);
    cJSON_Delete(root);
    // Free is intentional, it's client responsibility to free the result of cJSON_Print.
    free(post_data);
}

void mqtt_mandar_datos(float temperatura, bool nivAgua, float ec) {
    cJSON *root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "Temperatura", temperatura);
    cJSON_AddNumberToObject(root, "Nivel_agua", nivAgua);
